...
export function NotificationProvider({ children }) {
  ...
  const scheduleDonationReminder = async (donationDate) => {
    ...
    const identifier = await NotificationService.scheduleDonationReminder(donationDate);
    if (identifier) {
  ...
  const scheduleNextDonationEligibility = async (lastDonationDate, gender) => {
    ...
    const identifier = await NotificationService.scheduleNextDonationEligibility(lastDonationDate, gender);
    if (identifier) {
      await refreshScheduledNotifications();
      return true;
    }
  ...
  const sendEmergencyNotification = async (bloodType, location) => {
    ...
    const identifier = await NotificationService.sendEmergencyNotification(bloodType, location);
    if (identifier) {
      await refreshScheduledNotifications();
      return true;
    }
